...
// 统一配置：删除速度（每字符 ms）与额外缓冲
const DELETE_PER_CHAR = 100
const EXTRA_BUFFER_MS = 200
function onTypingFinished() {
  // 注意：在 <script setup> 里 template 不会自动解包 ref
  const text = (hitokoto as any)?.value?.hitokoto ?? (hitokoto as any)?.hitokoto ?? ''
  const len = Array.from(String(text)).length // 兼容 emoji/组合字符
  const delay = len * DELETE_PER_CHAR + EXTRA_BUFFER_MS

  // 用 globalThis 避免 “setTimeout 不是实例属性” 的类型报错
          ...
          <div class="flex justify-center" :class="themeConfig.hero.socialStyle === 'separate' && 'sakura-social-card'">
            ...
            <span class="px-2 text-lg">
              <template v-if="themeConfig.hero.typewriter">
                ...
                <SakuraTypewriter
                    v-if="themeConfig.hero.typewriter && banner.enableHitokoto"
                    :key="hitokoto?.id || hitokoto?.hitokoto"
                    :type-string="hitokoto?.hitokoto"
                    loop
                    :delay="100"
                    :pause-for="10000"
                    :delete-all="DELETE_PER_CHAR"
                    @all-typing-finished="onTypingFinished"
                />
              </template>
              <template v-else>
