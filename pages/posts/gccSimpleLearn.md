...
`static` 表示某个成员变量或成员函数是静态的。
- 修饰全局变量时， `static` 变量具有内部链接性，即只能在当前文件中访问，其他文件无法访问。
- 修饰局部变量时，`static` 变量在函数调用结束后不会被销毁（被放到了静态存储区而不是栈区），会一直存在于内存中，直到程序结束。
- 修饰类成员函数时，可以在其他文件中访问，通过直接用 **类名::函数名** 的方式访问。其限制在于它没有 `this` 指针，不属于任何具体的对象。
- 修饰类成员变量时，`static` 类变量必须在类外进行初始化。单例模式中（局部静态变量），在第一次被使用时触发构造函数，在程序结束后执行析构函数。在类中的 `static` 成员变量是所有对象共享的。
